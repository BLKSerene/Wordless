...
def wl_lemmatize_text(main, text, lang, lemmatizer):
    ...
    # Stanza
        ...
        for line in text.splitlines(keepends = True):
            # simplemma
            if lemmatizer.startswith('simplemma_'):
                tokens_line = wl_word_tokenization.wl_word_tokenize_flat(main, line.strip(), lang = lang)
            ...
            # Russian & Ukrainian
                ...
                for token in wl_word_tokenization.wl_word_tokenize_flat(main, line.strip(), lang = lang):
                    tokens.append(str(token))
                    lemmas.append(morphological_analyzer.parse(token)[0].normal_form)
            # Tibetan
            elif lemmatizer in ('botok_xct', 'modern_botok_bod'):
                for token in main.__dict__[f'{lemmatizer[:-4]}_word_tokenizer'].tokenize(line.strip()):
                    tokens.append(token.text)
