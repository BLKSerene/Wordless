        ...
        for line in text.splitlines(keepends = True):
            match pos_tagger:
                ...
                case 'nltk_perceptron_eng' | 'nltk_perceptron_rus':
                    tokens = wl_word_tokenization.wl_word_tokenize_flat(main, line.strip(), lang = lang)
                ...
                # Japanese
                case 'sudachipy_jpn':
                    for token in main.sudachipy_word_tokenizer.tokenize(line.strip()):
                        tokens_tagged.append(token.surface())
                        tags.append('-'.join((pos for pos in token.part_of_speech()[:4] if pos != '*')))
                # Khmer
                case 'khmer_nltk_khm':
                    for token, tag in khmernltk.pos_tag(line.strip()):
                        tokens_tagged.append(token)
                        tags.append(tag)
                # Korean
                case 'python_mecab_ko_mecab':
                    for token, tag in main.python_mecab_ko_mecab.pos(line.strip()):
                        tokens_tagged.append(token)
                        tags.append(tag)
                # Lao
                ...
                # Tibetan
                case 'botok_xct':
                    for token in main.__dict__['botok_word_tokenizer'].tokenize(line.strip()):
                        tokens_tagged.append(token.text)
                        tags.append(token.pos if token.pos else token.chunk_type)
                # Vietnamese
                case 'underthesea_vie':
                    for token, tag in underthesea.pos_tag(line.strip()):
                        tokens_tagged.append(token)
                        tags.append(tag)
