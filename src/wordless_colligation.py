                ...
                for i, ngram in enumerate(nltk.ngrams(tokens, ngram_size)):
                    for j, collocate in enumerate(reversed(text.tags_pos[max(0, i - window_size_left) : i])):
                        if wordless_matching.check_context(i, tokens,
                            ...
                            colligations_freqs_file[(ngram, collocate)][window_size_left - 1 - j] += 1
                    ...
                    for j, collocate in enumerate(text.tags_pos[i + ngram_size: i + ngram_size + window_size_right]):
                        if wordless_matching.check_context(i, tokens,
                            ...
                            colligations_freqs_file[(ngram, collocate)][window_size_left + j] += 1
...
def generate_table(main, table):
    def data_received(colligations_freqs_files, colligations_stats_files, nodes_text):
        if colligations_freqs_files:
            ...
            for i, ((node, collocate), stats_files) in enumerate(wordless_sorting.sorted_collocations_stats_files(colligations_stats_files)):
