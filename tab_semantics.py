...
def init(self):
    def search_settings_changed():
        ...
        table_semantics.setHorizontalHeaderItem(2, QTableWidgetItem(self.settings['semantics']['search_for']))
...
def search(self, table):
    def append_search_result(row, synset):
        ...
        table.setItem(row, 1, QTableWidgetItem(self.settings['semantics']['parts_of_speech'][synset.pos()]))
    ...
    if self.settings['semantics']['search_term']:
        table.clear_table()
        table.setRowCount(0)

        i = 0
        for synset in nltk.corpus.wordnet.synsets(self.settings['semantics']['search_term']):
            if self.settings['semantics']['search_for'] == 'Synonyms':
                append_search_result(i, synset)

                i += 1
            elif self.settings['semantics']['search_for'] == 'Antonyms':
                for lemma in synset.antonyms():
                    for antonym in lemma.antonyms():
                        append_search_result(i, antonym.synset())

                        i += 1
            elif self.settings['semantics']['search_for'] == 'Hypernyms':
                for hypernym in synset.hypernyms():
                    append_search_result(i, hypernym)

                    i += 1
            elif self.settings['semantics']['search_for'] == 'Hyponyms':
                for hyponym in synset.hyponyms():
                    append_search_result(i, hyponym)

                    i += 1
            elif self.settings['semantics']['search_for'] == 'Member Holonyms':
                for member_holonym in synset.member_holonyms():
                    append_search_result(i, member_holonym)

                    i += 1
            elif self.settings['semantics']['search_for'] == 'Member Meronyms':
                for member_meronym in synset.member_holonyms():
                    append_search_result(i, member_meronym)

                    i += 1
            elif self.settings['semantics']['search_for'] == 'Part Holonyms':
                for part_holonym in synset.part_holonyms():
                    append_search_result(i, part_holonym)

                    i += 1
            elif self.settings['semantics']['search_for'] == 'Part Meronyms':
                for part_meronym in synset.part_holonyms():
                    append_search_result(i, part_meronym)

                    i += 1
            elif self.settings['semantics']['search_for'] == 'Substance Holonyms':
                for substance_holonym in synset.substance_holonyms():
                    append_search_result(i, substance_holonym)

                    i += 1
            elif self.settings['semantics']['search_for'] == 'Substance Meronyms':
                for substance_meronym in synset.substance_holonyms():
                    append_search_result(i, substance_meronym)

                    i += 1
            elif self.settings['semantics']['search_for'] == 'Entailments':
...
def generate_plot(self):
    def search(graph, search_item):
        ...
        if self.settings['semantics']['search_for'] == 'Synonyms':
            ...
            if self.settings['semantics']['search_for'] == 'Hypernyms':
            ...
            for i, synset in enumerate(synsets):
                if self.settings['semantics']['depth_no_limit'] or i < self.settings['semantics']['depth_max']:
                    graph.add_edge(search_item.name(), synset.name())
                    graph.depth[synset.name()] = graph_depth + 1

                    if self.settings['semantics']['show_lemmas']:
                        for j, lemma in enumerate(synset.lemma_names()):
                            if self.settings['semantics']['depth_no_limit'] or j < self.settings['semantics']['depth_max']:
                                graph.add_edge(synset.name(), lemma)
                                graph.depth[lemma] = graph_depth + 2

                    if (self.settings['semantics']['recursive'] and self.settings['semantics']['depth_no_limit'] or
                        self.settings['semantics']['recursive'] and graph_depth <= self.settings['semantics']['depth_max']):
                        search(graph, synset)
    ...
    if self.settings['semantics']['search_term']:
        ...
        graph.depth = {'Root: ' + self.settings['semantics']['search_term']: 0}
        
        lemmas = nltk.corpus.wordnet.lemmas(self.settings['semantics']['search_term'])
        for lemma in lemmas:
            graph.add_edge('Root: ' + self.settings['semantics']['search_term'], lemma.synset().name())
