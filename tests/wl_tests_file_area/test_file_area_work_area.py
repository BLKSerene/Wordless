# ----------------------------------------------------------------------
# Wordless: Tests - File Area - Work Area
...
def test_file_area():
    ...
    for file_path in glob.glob('tests/files/wl_file_area/work_area/*.txt'):
        ...
        assert new_file['path'] == wl_paths.get_normalized_path(file_path).replace(os.path.join('tests', 'files', 'wl_file_area', 'work_area'), 'imports')
    ...
    for file_path in glob.glob('tests/files/wl_file_area/work_area/*.txt'):
        ...
        assert new_file['path'] == wl_paths.get_normalized_path(file_path).replace(os.path.join('tests', 'files', 'wl_file_area', 'work_area'), 'imports').replace('.txt', ' (2).txt')
        assert new_file['path_original'] == wl_paths.get_normalized_path(file_path)
        assert new_file['encoding'] == 'utf_8'
        assert new_file['lang'] == re.search(r'(?<=\[)[a-z_]+(?=\])', file_path).group()
        assert not new_file['tokenized']
